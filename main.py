...
def main(dsname,epoch,seed):
    ...
    model_edge,loss_acc_actual_pred,final,best_acc = train(model_edge,train_loader,val_loader,test_loader,epoch,float(defaults['lr_edge']),float(defaults['weight_decay']))

    print("\n\n########################### Save Logs ##################################\n")
    print(final,"\n")
    save_logs(model_edge,loss_acc_actual_pred,defaults,defaults['log_path'],epoch,dsname,seed,best_acc)
