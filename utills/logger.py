...
import os
def save_logs(model_edge,loss_acc_actual_pred,defaults,log_path,epoch,dsname,seed,accuracy):
    ...
    model_file_GPU=os.path.join(log_path,f'{dsname}_Sensory_model_Linear_GPU_{round(accuracy,3)}_{formatted_datetime}.pkl')
    model_file_CPU=os.path.join(log_path,f'{dsname}_Sensory_model_Linear_CPU_{round(accuracy,3)}_{formatted_datetime}.pkl')
    stat_file=os.path.join(log_path,f'{dsname}_stats_Results_Linear_{round(accuracy,3)}_{formatted_datetime}.txt')
